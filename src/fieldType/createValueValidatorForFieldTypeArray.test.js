...
const createValueValidatorForFieldTypeArray = require('./createValueValidatorForFieldTypeArray')
...
test('Can create a field type array value validator that correctly assesses validity.', () => {
  ...
  const validator = createValueValidatorForFieldTypeArray(ajv, fieldType, [fieldType], [])
...
test('Fail to create a field type value validator for a field with invalid schema.', () => {
  ...
  expect(() => createValueValidatorForFieldTypeArray(ajv, fieldType, [fieldType], []))
    .toThrow(/Unable to create field value array validator for 'candidateFieldType'/)
})
